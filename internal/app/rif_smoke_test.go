...
func TestResearchDiagnosticsSmokeProvesCompleteIncompleteAndRejectedBehavior(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if tc.name == "complete_candidate" && diagnostic.ArtifactHash != "sha256:72e84194a85be810b50b8de096ef09338c7f45e237282c2d924839bd767e61bc" {
...
func TestResearchDiagnosticsCanonicalEvidenceIsOrderIndependentAndValueBound(t *testing.T) {
	...
	firstConfig, err := researchidentity.ResolveConfiguration(context, []byte(`{"diagnostic_minimum_samples":30,"gate_thresholds":{"minimum_events":300}}`))
	if err != nil {
		t.Fatal(err)
	}
	secondConfig, err := researchidentity.ResolveConfiguration(context, []byte(`{"gate_thresholds":{"minimum_events":300},"diagnostic_minimum_samples":30}`))
	if err != nil {
		t.Fatal(err)
	}
	changedConfig, err := researchidentity.ResolveConfiguration(context, []byte(`{"gate_thresholds":{"minimum_events":301},"diagnostic_minimum_samples":30}`))
	if err != nil {
		t.Fatal(err)
	}
